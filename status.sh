...
# 3. Проверка Docker (пропускаем, используется внешняя БД)
log "🐳 Проверка Docker..."
echo "   ℹ️  Docker не проверяется (используется внешняя база данных)"
